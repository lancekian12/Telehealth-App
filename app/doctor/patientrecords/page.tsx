...
function PatientCard({
  ...
  return (
    <button
      onClick={onClick}
      className={[
        "group flex w-full items-start gap-3 rounded-2xl border p-3 text-left transition-all sm:items-center sm:gap-4 sm:p-4",
        active
    ...
    >
      {patient.avatar ? (
        <img
          alt={patient.name}
          src={patient.avatar}
          className="h-11 w-11 shrink-0 rounded-full object-cover ring-2 ring-primary/20 sm:h-12 sm:w-12"
        />
      ) : (
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-slate-100 font-bold text-slate-600 sm:h-12 sm:w-12">
          {patient.name
            .split(" ")
            .map((n) => n[0])
            .slice(0, 2)
      ...
      <MoreVertical className="mt-1 h-5 w-5 shrink-0 text-slate-300 group-hover:text-primary sm:mt-0" />
    </button>
  );
}
...
function SelectedPatientContent({
  patient,
  editingPrescription,
  ...
  patient: PatientRow;
  editingPrescription: boolean;
  setEditingPrescription: (value: boolean) => void;
  prescriptionForm: Prescription;
  setPrescriptionForm: React.Dispatch<React.SetStateAction<Prescription>>;
  ...
  return (
    <>
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        />
        <InfoCard label="Status" value={patient.lastAppointment?.status || "—"} />
      </div>
      ...
      <div className="mt-6 space-y-6 sm:mt-8 sm:space-y-8">
        <section>
          ...
          <div className="grid gap-4 rounded-3xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2 sm:p-5">
            <DetailItem
              icon={<Calendar size={16} />}
              title="Schedule"
              value={formatTimeRange(patient.startTime, patient.endTime)}
            />
            <DetailItem
        ...
        <section>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {!editingPrescription ? (
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 sm:w-auto"
              >
                <Edit3 size={16} />
                {patient.prescription ? "Edit prescription" : "Add prescription"}
              </button>
            ) : (
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                ...
                <button
                  ...
                  className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-white shadow-lg shadow-primary/20 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-70 sm:w-auto"
                >
                  {savingPrescription ? (
          ...
          {editingPrescription ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
              ...
              <span className="font-semibold">Add prescription</span> to create
              one.
            </div>
          )}
        </section>
...
export default function DoctorPatientRecord() {
  ...
  function resetPrescriptionToCurrent() {
    setEditingPrescription(false);
    setSaveError(null);
    setSaveMessage(null);
    setPrescriptionForm({
    ...
    <div className="min-h-dvh overflow-x-hidden bg-white text-slate-900">
      ...
      <div className="md:hidden">
        <div className="mx-auto flex min-h-dvh w-full flex-col px-3 py-3">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 max-h-[38dvh] overflow-y-auto pr-1 mobile-scroll">
              {loading ? (
          ...
          <section className="mt-4 min-h-0 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            {!patient ? (
              <div className="flex min-h-[260px] items-center justify-center p-6 text-center text-slate-500">
                Select an appointment record to view details.
              </div>
            ) : (
              <div className="flex flex-col">
                <div className="bg-gradient-to-r from-primary/10 via-slate-100 to-primary/5 px-4 py-4">
                  <div className="flex items-start gap-3">
                    {patient.avatar ? (
                      <img
                        alt={patient.name}
                        className="h-16 w-16 rounded-3xl object-cover shadow-lg ring-4 ring-white"
                        src={patient.avatar}
                      />
                    ) : (
                      <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-slate-100 text-lg font-bold text-slate-600 shadow-lg ring-4 ring-white">
                        {patient.name
                          .split(" ")
                          .map((n) => n[0])
                          .slice(0, 2)
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-sm text-slate-500">{patient.meta}</p>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {patient.phone ? (
                      <button className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600">
                        <Phone size={16} />
                        Call
                      </button>
                    ) : (
                ...
                <div className="p-4">
                  <SelectedPatientContent
                    ...
                    onResetPrescription={resetPrescriptionToCurrent}
                  />
                </div>
      ...
      <div className="hidden md:block">
        <div className="mx-auto flex min-h-dvh w-full max-w-7xl flex-col px-3 py-3 sm:px-6 sm:py-4 lg:px-8">
          ...
          <main className="grid flex-1 gap-4 lg:grid-cols-[380px_1fr]">
            <aside className="flex min-h-0 flex-col rounded-3xl border border-slate-200 bg-white p-4 shadow-sm lg:max-h-[calc(100dvh-8rem)]">
              <div className="relative">
                <Search
                  size={18}
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"
            ...
            <section className="min-h-0 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              {!patient ? (
                <div className="flex h-full min-h-[420px] items-center justify-center p-6 text-slate-500 sm:min-h-[500px] sm:p-8">
                  Select an appointment record to view details.
                </div>
              ) : (
                <div className="relative flex h-full flex-col">
                  ...
                  <div className="relative z-10 border-b border-slate-100 px-4 pb-5 pt-4 sm:px-6 sm:pb-6 sm:pt-6">
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div className="flex gap-4">
                        {patient.avatar ? (
                          <img
                            alt={patient.name}
                            className="h-20 w-20 rounded-3xl object-cover shadow-lg ring-4 ring-white sm:h-24 sm:w-24"
                            src={patient.avatar}
                          />
                        ) : (
                          <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-slate-100 text-xl font-bold text-slate-600 shadow-lg ring-4 ring-white sm:h-24 sm:w-24 sm:text-2xl">
                            {patient.name
                              .split(" ")
                              .map((n) => n[0])
                              .slice(0, 2)
                      ...
                      <div className="flex flex-wrap gap-3 md:justify-end">
                        {patient.phone ? (
                          <button className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 sm:flex-none">
                            <Phone size={16} />
                            Call
                          </button>
                        ) : null}
                  ...
                  <div className="custom-scrollbar flex-1 overflow-y-auto p-4 sm:p-6 sm:pb-8">
                    <SelectedPatientContent
                      patient={patient}
                      editingPrescription={editingPrescription}
                      setEditingPrescription={setEditingPrescription}
